...
export default function TraySpecificationConverter() {
  ...
  function download() {
    const header = ["Поз.", "Производитель", "Серия", "Исходное наименование", "Исходный артикул", "Распознанные характеристики", "Не определено", "Наименование KLM", "Код подбора KLM", "Кол-во", "Ед.", "Уверенность"];
    const body = rows.map((row) => [row.position, row.manufacturer, row.series, row.name, row.article, row.characteristics.join(", "), row.missingCharacteristics.join(", "), row.klmName, row.klmArticle, row.klmQuantity, "шт", `${row.confidence}%`]);
        ...
        <div className="overflow-hidden rounded-xl2 border border-line bg-surface shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] text-left text-[12px]">
              <thead className="bg-bg text-mute"><tr>{["Поз.", "Конкурент и характеристики", "Аналог KLM", "Код подбора KLM", "Кол-во", "Статус"].map((head) => <th key={head} className="px-4 py-3 font-bold">{head}</th>)}</tr></thead>
                ...
                {rows.map((row, index) => (
                  <tr key={`${row.position}-${index}`} className="align-top hover:bg-[#f8fbfc]">
                    ...
                    <td className="max-w-[340px] px-4 py-4">
                      ...
                      <p className="mt-1 text-mute">{row.article || "Без артикула"}</p>
                      {row.catalogName && <p className="mt-1 text-[11px] leading-snug text-mute">Каталог: {row.catalogName}</p>}
                      <p className="mt-1 font-bold text-cur-d">{row.manufacturer}{row.series ? ` · ${row.series}` : ""}</p>
                      <div className="mt-2 flex flex-wrap gap-1">
                        {row.characteristics.map((item) => <span key={item} className="rounded-full bg-cur-soft px-2 py-0.5 text-[10px] font-semibold text-cur-d">{item}</span>)}
                      </div>
                      {row.missingCharacteristics.length > 0 && <p className="mt-2 text-[10px] font-semibold text-[#996600]">Не определено: {row.missingCharacteristics.join(", ")}</p>}
                    </td>
