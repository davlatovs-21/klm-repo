...
export const MANUFACTURERS = [
  ...
  { name: "IEK", aliases: /\b(iek|иэк|esca)\b/i, article: /^(clp|cln|clw|cta|cpo|ct)/i },
...
const DESIGN = [
  { value: "solid" as const, label: "неперфорированный", code: "LN", words: /неперфор|глухой|solid/i }, { value: "wire" as const, label: "проволочный", code: "LPV", words: /проволоч|wire/i },
  { value: "ladder" as const, label: "лестничный", code: "LL", words: /лестнич|ladder/i }, { value: "perforated" as const, label: "перфорированный", code: "LP", words: /перфор|perforat/i },
];
...
function detectThickness(text: string) { const match = text.match(/(?:толщ(?:ина)?\s*[:=]?|\b[stт]\s*=|[-(]\s*)(\d(?:[.,]\d+)?)\s*мм\b/i); return match ? Number(match[1].replace(",", ".")) : null; }
...
export function convertRow(row: SourceRow): ConvertedRow {
  ...
  const confidence = catalogMatch ? (complete ? 99 : 90) : complete ? 94 : requiredKnown ? 78 : kind !== "unknown" ? 55 : 20, meters = /(^|\s)(м|м\.|метр)/i.test(row.unit), klmQuantity = kind === "tray" && meters ? Math.ceil((row.quantity * 1000) / (standardLength || 3000)) : row.quantity;
