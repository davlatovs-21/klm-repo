...
const PAIN = [
  {
    ...
    d: "Инженер сверяет серию, ток магистрали, ток отвода, совместимость КОМ и IP по нескольким таблицам печатного каталога.",
  },
  {
    Ico: IconCode,
    t: "Строка заказа вручную",
    d: "Артикул собирается позиция за позицией. Один перепутанный сегмент — и в заказ уходит другое изделие.",
  },
  {
    Ico: IconFactory,
...
const STEPS = [
  ...
  { n: "02", Ico: IconTap, t: "Отвод", d: "Ток отвода из ряда КОМ, число отводов, рукоятка, IP корпуса. Аппарат защиты следует из номинала корпуса." },
  { n: "03", Ico: IconShield, t: "Проверка", d: "Правила справочника с числами в формулировке нарушения и кнопкой исправления на один клик." },
  { n: "04", Ico: IconGauge, t: "Результат", d: "Корпус КОМ, строка заказа, загрузка корпуса по току, рекомендации по монтажу, печать и ссылка." },
];

/* список правил берётся из ядра, чтобы страница не расходилась с расчётом */
const RULES: [string, string][] = runChecks(DEFAULT_CONFIG).checks.map((c) => [c.name, c.info ?? c.text]);
        ...
        <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-14 sm:px-5 sm:py-20 lg:grid-cols-[1.05fr_1fr] lg:items-center lg:py-24">
          <div className="anim-up">
            ...
            <p className="mt-5 max-w-xl text-[15.5px] text-[#8fb4c0] sm:text-[17px]">
              Инженер вводит параметры трассы и отвода. Панель проверяет конфигурацию по {RULES.length} правилам
              справочника, подбирает минимальный достаточный корпус КОМ и собирает строку заказа из {buildCode(DEFAULT_CONFIG).length} позиций.
            ...
            <dl className="mt-9 grid grid-cols-3 gap-3 border-t border-white/10 pt-6 sm:gap-4">
              {[
                ...
                [String(RULES.length), "правил проверки"],
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_1.1fr] lg:items-start">
          <div>
            <Head
              ...
              title={`${RULES.length} правил справочника, а не память инженера`}
            ...
            <div className="mt-6 rounded-xl2 border border-fault/25 bg-fault-soft p-5">
              ...
              <p className="mt-2 text-[14px] text-ink">
                «КОМ встают на ШРА KLM-R 250–1600 А, у вас 160 А»
      ...
      <section className="busgrid relative overflow-hidden bg-ink text-white">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-14 text-center sm:px-5 sm:py-20">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-[16px] text-[#8fb4c0]">
            Демо содержит рабочую логику подбора: те же четыре шага, те же правила, тот же формат строки заказа.
